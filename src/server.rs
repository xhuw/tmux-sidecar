...
use crate::{
    ipc::{
        ...
        ErrorMessage, HelloAck, ProjectionState, ServerMessage, SidecarPaths, StateUpdated,
    },
    model::WindowTarget,
    tmux::{Tmux, TmuxCli},
};
    ...
    while let Some(message) = crate::ipc::read_message(&mut reader)? {
        ...
        match message {
            ...
            ClientMessage::HookEvent(event) => {
                ...
                refresh_state(&shared, refresh_from_tmux, tmux.as_ref())?;
                send_message(
                    &writer,
...
mod tests {
    ...
    use crate::ipc::{
        ...
        ProjectionState, ServerMessage, SidecarPaths, StateUpdated,
    };
    use crate::{
